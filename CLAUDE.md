...
- **NIWA is the default UV source everywhere (as of 2026-08-18)** — both the on-screen chart/timeline (`app.js` `fetchUVData()`) and push notifications (`workers/uv-notifier/index.js` `fetchCurrentUV()`) use NIWA. The on-screen path falls back to Open-Meteo automatically if the NIWA fetch fails. The `?uvSource=niwa` toggle was removed — Open-Meteo is no longer the default anywhere, only the automatic failure fallback. This is ahead of the product/service licence requested in `docs/superpowers/specs/2026-08-07-niwa-licence-request-draft.md`; **NIWA's Access Terms permit internal/staff use only, so this default is only valid while this deployment has no visitors besides the maintainer — revisit before opening the app up to other users.** See `.claude/docs/api-notes.md` for the full source/path breakdown and licensing status.
